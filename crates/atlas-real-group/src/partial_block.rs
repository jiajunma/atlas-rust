...
impl IntegralSubsystem {
    ...
    pub fn integral(system: &RootSystem, gamma: &RationalWeight) -> Result<Self, StructureError> {
        ...
        let mut integral_roots = RootBits::new(system.roots().len());
...
/// Root set as a bit vector over the crate-internal `RootId` numbering:
/// the integral-subsystem basis reduction inserts/contains/removes/iterates
/// per call, and BTreeSet's node allocations and pointer-chasing iteration
/// showed up in the unitarity profile. Iteration is ascending `RootId`,
/// exactly the BTreeSet order it replaces.
#[derive(Clone)]
struct RootBits {
    words: Vec<u64>,
}
...
impl RootBits {
    fn new(root_count: usize) -> Self {
        Self {
            words: vec![0_u64; root_count.div_ceil(64)],
        }
    }

    fn insert(&mut self, id: RootId) {
        self.words[id.index() / 64] |= 1_u64 << (id.index() % 64);
    }
    ...
    fn contains(&self, id: RootId) -> bool {
        self.words[id.index() / 64] >> (id.index() % 64) & 1 == 1
    }

    fn iter(&self) -> impl Iterator<Item = RootId> + '_ {
        self.words.iter().enumerate().flat_map(|(word_index, &word)| {
            ...
            std::iter::from_fn(move || {
                ...
                word &= word - 1;
                Some(RootId::from_usize(word_index * 64 + bit))
            })
        })
    }
}
...
fn simple_basis(
    ...
    'outer: for alpha in subset.iter() {
