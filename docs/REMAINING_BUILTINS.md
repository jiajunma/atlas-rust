...
| 5 | W-cells and KL access: W_cells, W_graph, KL_column, raw_KL, raw_ext_KL, dual_KL, KL_sum_at_s, KL_sum_at_s_to_height, twisted_KL_sum_at_s | PARTIAL: W_cells/W_graph(Param) done (`domain/w_graph_param`), raw_KL/dual_KL done (`domain/raw_kl`), all HPC-verified `3507974`. KL_column needs the Rep_table contributions/singular machinery; KL_sum_at_s* evaluate the full KL column at q=s with contributions. |
