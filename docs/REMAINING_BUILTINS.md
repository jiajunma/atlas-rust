...
| 6 | extended blocks: default_extended, extend, extended_block, finalize_extended, partial_block, partial_KL_block, partial_extended_KL_block, dual_KL_block, K_type_pol_extended, scale_extended | PARTIAL: partial_KL_block (`domain/partial_kl_block`, HPC `3508277` submitted) — the condensed KL matrix over the partial-block survivors with finals_for (blocks.cpp:335-368), first name landed. The rest need the ext_block layer. |
| 7 | deform variants: twisted_deform, twisted_full_deform, block_deform, full_deform, KL_block | PARTIAL: Rep_context::finals_for (repr.cpp:1205-1297, the parameter-level final decomposition with reflections/Cayley/inverse-Cayley and height-ordered insert) is ported (`0108799`), the core of full_deform; scale/deform_readjust/deformation_terms already exist. full_deform still needs reducibility_points (repr.cpp:825-925) and the KTypePol path; the twisted variants need ext_block. |
