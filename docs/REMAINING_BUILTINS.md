...
| 3 | root/radical data | DONE: root_coradical, coroot_radical (`domain/radical`), components_rank, strong_components (`domain/components_rank`) all VERBATIM + HPC `3506900`. Only root_ladder_bottoms / coroot_ladder_bottoms remain (they need the root_perm/link permutations of rootdata.cpp:243-313 that RootTable does not store). |
...
| 5 | W-cells and KL access: W_cells, W_graph, KL_column, raw_KL, raw_ext_KL, dual_KL, KL_sum_at_s, KL_sum_at_s_to_height, twisted_KL_sum_at_s | PARTIAL: W_cells/W_graph(Param) (`domain/w_graph_param`), raw_KL/dual_KL (`domain/raw_kl`), KL_sum_at_s/_to_height (`domain/kl_sum_at_s`) all VERBATIM + HPC `3507974`/`3507981`. KL_column needs the partial-block `lookup`; twisted_KL_sum_at_s needs ext_block. Known: KlTable fails on B2 and higher fibred blocks whose real type-I Cayley images lie outside the block (graceful runtime error since `339530e`). |
