...
| 4 | print family: print_X, print_gradings, print_real_Weyl, print_block, print_blockd, print_blocku, print_blockstabilizer, print_common_block, print_KL_basis, print_prim_KL, print_KL_list, print_W_cells, print_W_graph | PARTIAL (8/13 HPC-verified `3506891`): print_block/blockd/blocku (`domain/block_print`), print_KL_basis/prim_KL/list, print_W_graph, print_W_cells (`domain/kl_print`) — all verbatim. Remaining: print_blockstabilizer/print_common_block need the common-block + realweyl layer; print_X needs the global KGB; print_gradings needs the Cartan fiber; print_real_Weyl needs realweyl |
| 5 | W-cells and KL access: W_cells, W_graph, KL_column, raw_KL, raw_ext_KL, dual_KL, KL_sum_at_s, KL_sum_at_s_to_height, twisted_KL_sum_at_s | PARTIAL: W_cells/W_graph(Param) (`domain/w_graph_param`), raw_KL/dual_KL (`domain/raw_kl`), KL_sum_at_s/_to_height (`domain/kl_sum_at_s`) all VERBATIM + HPC. The KL-table Cayley argument-order fix (`24ba188`) makes B2/G2 KL columns, raw_KL 12-element blocks, KL_sum_at_s B2, deform B2 and print_KL_basis B2 all match the oracle (fixtures extended, HPC `3508004`). KL_column needs the partial-block `lookup`; twisted_KL_sum_at_s needs ext_block. Known: KL_sum_at_s uses the input parameter's lambda-rho for every block element, so a parameter deep inside its block can hit a height-parity mismatch (fixtures use the block's lowest element). |
